...
public class DeviceChoiceActivity extends AppCompatActivity implements IActivityBasicSetting {
    ...
    void Click(View v) {
        switch (v.getId()) {
            case R.id.select_ergometer:
                Toast.makeText(getApplicationContext(), "에르고미터를 선택하셨습니다. 스캔 시작하겠습니다.", Toast.LENGTH_SHORT).show();
                startActivity(new Intent(DeviceChoiceActivity.this, EGOScanActivity.class));
                finish();
                break;

            case R.id.select_treadmill:
                Toast.makeText(getApplicationContext(), "트레드밀을 선택하셨습니다. 스캔 시작하겠습니다.", Toast.LENGTH_SHORT).show();
